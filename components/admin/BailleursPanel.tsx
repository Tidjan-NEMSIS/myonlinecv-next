...
export default function BailleursPanel() {
  const { cvData, setCvData, saveSection } = useCvData();
  const [newBailleur, setNewBailleur] = useState('');

  const handleAdd = () => {
    if (!newBailleur.trim()) return;
    setCvData(prev => ({
      ...prev,
      bailleurs: [...(prev.bailleurs || []), newBailleur.trim()]
    }));
    setNewBailleur('');
  };
  ...
  const handleRemove = (index: number) => {
    setCvData(prev => ({
      ...prev,
      bailleurs: prev.bailleurs.filter((_, i) => i !== index)
    }));
  };

  const handleSave = () => {
    saveSection({ bailleurs: cvData.bailleurs }, false);
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[var(--primary)] font-serif">Bailleurs de fonds & Partenaires</h2>
          <p className="text-[var(--text-secondary)]">Ajoutez les organisations avec lesquelles vous avez collaboré (ex: Banque Mondiale, USAID...).</p>
        </div>
      </div>
      ...
      <div className="bg-white/60 backdrop-blur-xl rounded-[2rem] shadow-sm border border-white/80 p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <input 
            type="text" 
            value={newBailleur}
            onChange={(e) => setNewBailleur(e.target.value)}
            ...
            placeholder="Ex: USAID, Union Européenne" 
          />
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {(!cvData.bailleurs || cvData.bailleurs.length === 0) && (
            <p className="text-gray-400 text-sm italic">Aucun bailleur ajouté.</p>
          )}
        </div>
